    ...
    fetch(apiUrl)
        ...
        .then(({ ok, data }) => {
            ...
            if (data && data.trend_1h) {
                ...
                if (typeof pictocode !== 'undefined') { const weatherInfo = translatePictocode(pictocode); weatherDescriptionElement.textContent = weatherInfo.description; weatherIconElement.src = weatherInfo.iconUrl; weatherIconElement.style.display = 'inline-block'; } else { weatherDescriptionElement.textContent = ''; weatherIconElement.style.display = 'none'; }
                if (time) { const [datePart, timePart] = time.split(' '); const [year, month, day] = datePart.split('-'); dateTimeElement.textContent = `${day}/${month}/${year} ${timePart}`; } else { dateTimeElement.textContent = 'No disponible'; }
